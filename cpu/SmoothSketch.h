...
public: 
	...
	void push(ID_TYPE id, uint32_t win, uint32_t* c) {
		...
		// assert(min_index >= 0);
		if (rand() % (win - min_start_window) == 0) {
...
public: 
	...
	SmoothSketch(uint32_t memory, double var_input, double error_input, double ratio_input, int CellNum, int S1Wins, double potential):win_cnt(0),last_timestamp(0){
