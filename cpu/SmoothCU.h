...
template<typename ID_TYPE>
class SmoothSketch_CU {
public: 
	SmoothSketch_CU() {}
	SmoothSketch_CU(uint32_t memory) : win_cnt(0), last_timestamp(0) {
	...
	SmoothSketch_CU(uint32_t memory, double var_input, double error_input, double ratio_input, int CellNum, int S1Wins, double potential):win_cnt(0),last_timestamp(0){
